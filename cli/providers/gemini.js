...
export class GeminiProvider extends AIProvider {
    ...
    /**
     * Generate PR title using Gemini
     */
    ...
    Return ONLY the PR title, no explanations or quotes.`;
        ...
        const result = await model.generateContent(prompt);
        return result.response?.text()?.trim() || desc;
    }
    ...
    Return ONLY the branch name, no explanations or quotes.`;
        ...
        const result = await model.generateContent(prompt);
        return result.response?.text()?.trim() || `feature/${desc.toLowerCase().replace(/\s+/g, '-')}`;
    }

    /**
     * Group files using Gemini AI analysis
     */
    async groupFilesWithAI(filesData, maxGroups = 5) {
        const spinner = ora('🧞 Analyzing changes with Gemini AI...').start();
...
- If scope is not applicable, use empty string ""`;
            ...
            spinner.succeed('✓ Gemini AI analysis complete');
            ...
            spinner.fail('Gemini AI analysis failed');
    ...
    /**
     * Generate commit message for a specific group using Gemini
     */
